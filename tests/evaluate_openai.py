"""Explicit Phase 7J synthetic-only endpoint evaluation; no production configuration wiring."""
import argparse
...
class SyntheticOpenAIProvider(CommandProvider):
    ...
    def __init__(self, scenario, key, model=MODEL):
        ...
        super().__init__(Path(__file__).with_name('openai_synthetic_adapter.py').resolve())
        self.environment.update(PHASE7J_SYNTHETIC_EVALUATION='1', PHASE7J_SCENARIO=scenario,
                                OPENAI_API_KEY=key.get_secret_value(), OPENAI_MODEL=model)
    ...
    async def _call(self, *args, **kwargs):
        ...
        if isinstance(value, dict) and 'evaluation_error' in value:
            from openai_synthetic_adapter import FAILURE_CODES
...
def local_configuration():
    ...
    model = os.environ.get('OPENAI_MODEL') or local.get('OPENAI_MODEL') or MODEL
...
def main(argv=None):
    ...
    if not key:
        print('NOT RUN: OPENAI_API_KEY is not locally configured. Preflight calls: 0. Generation calls: 0.')
        return 2
    if model != MODEL:
    ...
    with synthetic_client() as client:
        for index, scenario in enumerate(SCENARIOS):
            if index:
                time.sleep(5)
            provider = SyntheticOpenAIProvider(scenario, key, model)
